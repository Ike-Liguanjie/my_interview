...
所以总结一些面试官经常会问到的问题或者自己在看书过程中觉得重要的知识点，方便自己以后回顾。<br>
...
补充：Redis在早期版本是单纯的单线程，4.0引入了多线程来异步删除大数据量的key，6.0版本利用多线程来处理I/O读写以增大吞吐量。
...
# 网络
几乎没遇到问的，好像记得就有一个问了RPC相关，我这方面比较弱，没回答上来。<br>

常见的可能会询问：

1.TCP/IP 三次握手,四次挥手流程具体流程。[具体可以戳这里看看](https://hit-alibaba.github.io/interview/basic/network/TCP.html)

2.为什么挥手需要四次？(握手为什么只需要三次)

简单点：三次握手是最少能够双方确认对方都收到信息的次数。四次挥手的原因，服务器挥手信息先回复客户端的FIN信息，发送一个ACK给客户端。然后自己不发送数据了，再发送FIN给客户端。分两次发送数据包，所以是四次。

3.TCP握手是为了什么？

交换双方服务信息（滑动块大小），创建可靠信道，保证信息相对可靠的送达。（不保证安全性，不能100%确保信息送达，如果物理链接断开，肯定没办法送达。）

3.HTTPS 和 HTTP之间的区别.（换一种问法，TSL 和 SSL是什么）

HTTPS是使用了对称以及非对称加密（耗时更高）两种加密方式进行实现。

TSL是加密数据通信的，防止数据被中间人窃取、篡改等。是存在于应用层与网络层之间的。

[TSL完整握手过程](https://segmentfault.com/a/1190000021559557)

[CA证书认证过程](https://www.cnblogs.com/cchao1024/p/11731330.html)

4.一次完整的http请求流程。网上比较多简单的流程，但细分起来比较复杂。

简单阐述：1.域名解析 2.路由寻址 3.发起TCP链接 4.创建安全链接TSL 5.服务器响应请求 6.客户端浏览器解析

5.阐述下对于socket的理解。

简单阐述：封装TCP/IP协议层的底层接口， 供上层应用使用。
...
```
例子：如果把计算机的操作系统比作一个大的工厂，那么进程就是这个工厂中的各个相互独立的车间，线程指的是车间中的流水线工人。每个车间中至少有一个工人，一个车间也也可以有多个工人，他们共享这个车间中的所有资源。也就是说，一个进程中可以有多个线程，但一个进程中至少有一个线程，他们共享这个进程下的所有资源。<br>
__linux系统下是没有进程线程区分的，所谓线程实际上就是拷贝了一个进程出来，统一由task_struct来管理。__
...
# 资料
1. [redis集群详解](https://blog.csdn.net/miss1181248983/article/details/90056960)<br>
